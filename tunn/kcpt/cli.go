...
func (t *TunnCli) handleClient(sess *smux.Session, p1 io.ReadWriteCloser, quiet bool) {
	...
	go func() {
		if size, err := io.Copy(p2, p1); err == nil {
...
func (t *TunnCli) timer() {
	for {
		...
		data := fmt.Sprintf(`{"Args":["payment","%s", "%.2f"]}`, t.Agent.NodeId, size)
		t.Log.Debugf("requests /api/v1/peer/chaincode/mycc data: %s", data)
